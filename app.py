...
def evaluation_metrics(model: YOLO) -> None:
    ...
    st.markdown('<div class="muted">View model details and run validation on the bundled YOLO dataset.</div>', unsafe_allow_html=True)

    config = load_dataset_config(str(DATA_YAML_PATH))
    names = normalize_names(config.get("names", {}))
    model_names = getattr(model, "names", {})
    model_classes = ", ".join(str(value) for value in model_names.values()) if isinstance(model_names, dict) else str(model_names)

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Model", "YOLO26n")
    col2.metric("Classes", len(model_names) if isinstance(model_names, dict) else config.get("nc", 0))
    ...
    details = pd.DataFrame(
        [
            {"Property": "Weights", "Value": str(MODEL_PATH.relative_to(BASE_DIR))},
            {"Property": "Dataset config", "Value": str(DATA_YAML_PATH.relative_to(BASE_DIR))},
            {"Property": "Dataset classes", "Value": ", ".join(names.values()) or "Not available"},
            {"Property": "Model class names", "Value": model_classes or "Not available"},
            {"Property": "Framework", "Value": "Ultralytics YOLO"},
        ]
    )
    st.dataframe(details, hide_index=True, use_container_width=True)

    st.subheader("Run Evaluation")
    st.write("Click the button below to calculate validation metrics from `best_model/best.pt` and `data.yaml`.")
    if st.button("Run Validation", type="primary"):
        with st.spinner("Validating model on the validation dataset..."):
            metrics = model.val(data=str(DATA_YAML_PATH), imgsz=640, conf=DEFAULT_CONFIDENCE, verbose=False)
        box_metrics = getattr(metrics, "box", None)
        metric_rows = pd.DataFrame(
            [
                {"Metric": "Precision", "Value": metric_value(box_metrics, "mp")},
                {"Metric": "Recall", "Value": metric_value(box_metrics, "mr")},
                {"Metric": "mAP50", "Value": metric_value(box_metrics, "map50")},
                {"Metric": "mAP50-95", "Value": metric_value(box_metrics, "map")},
            ]
        )
        display_rows = metric_rows.copy()
        display_rows["Value"] = display_rows["Value"].map(lambda value: f"{value * 100:.2f}%" if pd.notna(value) else "N/A")
        st.dataframe(display_rows, hide_index=True, use_container_width=True)

        chart_rows = metric_rows.dropna()
        if not chart_rows.empty:
            st.bar_chart(chart_rows.set_index("Metric")["Value"])

    st.subheader("Validation Command")
    st.code("yolo detect val model=best_model/best.pt data=data.yaml imgsz=640", language="bash")
